...
def load_config():
    ...
    if not config:
        raise Exception(
            'Unable to load config contents from %s' % default_config_filename)
...
def process_objc(config):
    ...
    objc_files = [
        ...
        for dirpath, _, files in os.walk(config['objcRootDir']) for f in files
        if re.match(r'.+\.(m|h|swift)$', f)
    ]
    ...
    for strings_file in glob.glob(os.path.join(temp_dir, '*.strings')):
        with codecs.open(strings_file, 'r', 'utf-16-le') as infile:
            with codecs.open(
                    os.path.join(config['enLprojDir'],
                                 os.path.basename(strings_file)), 'w',
                    'utf-8') as outfile:
...
PLIST_STRING_KEYS = [
    'Title', 'ShortTitle', 'FooterText', 'IASKSubtitle', 'IASKPlaceholder'
]
...
def process_all_plists(config):
    ...
    with codecs.open(
            os.path.join(config['enLprojDir'], 'Root.strings'), 'w',
            'utf-8') as strings_file:
        strings_file.write('/* THIS FILE IS GENERATED. DO NOT EDIT. */\n\n')
        for key in strings:
            strings_file.write('/* %s */\n"%s" = "%s";\n\n' %
                               (strings[key]['description'],
                                strings[key]['key'], strings[key]['default']))
